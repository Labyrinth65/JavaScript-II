...
let fullName = [];
runners.forEach((el, i) => fullName.push(`${el.first_name} ${el.last_name}`));
console.log(fullName);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = [];
runners.map((el) => allCaps.push(`${el.first_name.toUpperCase()} ${el.last_name}`));
console.log(allCaps); 
...
let ticketPriceTotal = runners.reduce(((t, a) => t + a.donation),0);
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1, count shirt sizes, put total number into array
let shirtSizes = 
...
let companyList = 
runners.map((el) => el.company_name)
.reduce(function(a,b){
    if(a.indexOf(b)<0) 
    a.push(b); 
    return a;
